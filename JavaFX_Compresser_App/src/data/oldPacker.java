 ...
 */
public class oldPacker {

    private static final int MINIMUM_REPEATING_LENGTH = 5; // < 5 gir større, > 5 gir større. 5 er sweetspot

    private byte[] allBytes;

    public oldPacker(byte[] array) {

        allBytes = array;

    }

    public byte[] pack() {return pack(Byte.MAX_VALUE-MINIMUM_REPEATING_LENGTH);}
    public byte[] pack(int partitionSize) {
        ...
        for (int i = 0; i < partitions.length; i++) { // dette var dumt, skulle ikke ha gjort dette
    ...
    public byte[] lempelZivCompress(byte[] array) {
        ...
        byte[] completeCompressedArray = new byte[array.length+(int)(Math.ceil((double)array.length / (double) Byte.MAX_VALUE))]; // worst case nothing can be compressed it will be the length + codes
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            boolean lengthIsMin = lengthToCopy == MINIMUM_REPEATING_LENGTH; // Search må være Min length, ellers får vi edgecase feil

            System.arraycopy(array,i,search,0,lengthToCopy);

            //System.out.println("search is: "+new String(search));

            if (ArrayUtils.contains(completeCompressedArray/*compressed*/, search)
                    || ArrayUtils.contains(uncompressedBytes, search)
                            && lengthIsMin) { // burde heller søkt bakover her enn å ha partisjoner
                ...
                System.arraycopy(uncompressedBytes, 0, completeCompressedArray, completeCompressedArrayCounter, uncompressedBytes.length);
                completeCompressedArrayCounter += uncompressedBytes.length;
                uncompressedCounter = 0;
                uncompressedBytes = new byte[Byte.MAX_VALUE];
                // Finished

                search = ArrayUtils.trim(findLargestRepeatable(array, completeCompressedArray/*compressed*/, search, i));
                if (search.length < 1) continue;
                //System.out.println("Largest repeat is : "+ Arrays.toString(search));

                byte[] code = generateBackwardsCode(completeCompressedArray, search, completeCompressedArrayCounter);

                //System.out.println("code "+Arrays.toString(code)+ " - current: "+completeCompressedArrayCounter+" - i:"+i+" - "+Arrays.toString(completeCompressedArray)+"\n"+ Arrays.toString(search));
    ...
    public byte[] findLargestRepeatable(byte[] all, byte[] compressed, byte[] search, int indexOfSearch) {
        ...
        while (ArrayUtils.contains(compressed, search)) { // Legger til 1 til det ikke matcher lenger
     ...
     * @param whereYouAreNow          the next index in the compressed array
     * @return an array with code for length backwards at 0, and length of the word at 1
     */
    public byte[] generateBackwardsCode(byte[] compressed, byte[] search, int whereYouAreNow) {
        ...
        return new byte[]{(byte) -(whereYouAreNow - (index)), length};
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("Original: "+bytes.length+" - Compressed: "+compressed.length);
