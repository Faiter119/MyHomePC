...
import static utils.FXUtil.clearAll;
import static utils.TimeUtil.parseBadlyFormattedTime;
...
public class EventSaverApp extends Application { // TODO: 07.07.2016 cleanup
    ...
    public void start(Stage theStage) {
        ...
        stage = theStage;
        //events = read---
        Optional<Object> oEvents = FileManager.read();

        oEvents.ifPresent(o -> events = (ArrayList<Event>) o);
        //events = (ArrayList<Event>) oEvents.get();
        //FileManager.read().ifPresent(System.out::println);
        System.out.println(FileManager.getJarFolder());
        if(events == null){
            events = External.noStorageFileFoundAlert(stage);

            //events = gui.External.loadNewEvents(stage);

            if(events == null) {
                events = FileManager.getDefaultEvents();
                /*System.out.println("i give up");
                System.exit(-1); // just give up*/
            }
        }
        
        if(events.size() == 0) events = FileManager.getDefaultEvents();
        ...
        stage.setOnCloseRequest( (event) -> FileManager.getJarFolder().ifPresent((consumer) -> FileManager.write(new File(consumer, "storage.txt"), events)) );
        stage.show();
    }
    ...
    public static Pane newEventPane(){
       ...
       /* TextField dateTextField = new TextField(LocalDate.now().toString());
        ...
        addNewEventButton.setOnAction((e)->{
            ...
            try{
                ...
                if(!startString.trim().equals("")) {
                    event.setStart(parseBadlyFormattedTime(startString));
                }

                if(!endString.trim().equals("")) {
                    event.setEnd(parseBadlyFormattedTime(endString));
                }
        ...
        Label label = new Label("New backend.Event");
        label.setFont(new Font("Consolas",20));
    ...
    public static Pane tableButtonsPane(){
        ...
        // Delete
            Label deleteLabel = new Label("Type \"delete\" to delete backend.Event. This is final!");
        ...
        // Load
            ...
            loadButton.setOnAction(event -> {
                ...
                if(newEvents != null) {
                    ...
                    FileManager.getJarFolder().ifPresent( (consumer) -> FileManager.write(new File(consumer, "storage.txt"),events)); // dam son where'd you find this
    ...
    public static Pane eventEditPane(Event event){
        ...
        Label label = new Label("Edit backend.Event");
        label.setFont(new Font("Consolas",20));
        ...
        acceptButton.setOnAction(e -> {
            ...
            if(!startOrig.equals(startNew)){
                event.setStart(parseBadlyFormattedTime(startNew));
            }
            if(!endOrig.equals(endNew)){
                event.setEnd(parseBadlyFormattedTime(endNew));
            }
